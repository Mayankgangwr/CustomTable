...
import { FilterOperationsEnum, IAppliedFilter, IFilterOperations, ISort, ISortState, ITableItemSearchState, ITableRows } from "../Interface";
import moment from "moment";

// its handling sorting of all columns if sort method to be update 
export const handleSort = async (key: string, isSort: ISortState, tableRows: ITableRows[] | null): Promise<{
    isSort: ISortState;
    tableRows: ITableRows[] | null;
}> => {
...
export const handleSortTable = ({ key, sortBy }: { key: string; sortBy: ISort }, tableRows: ITableRows[] | null): ITableRows[] | null => {
...
export const handleFilter = (tableRows: ITableRows[], column: string, value: string, operation: FilterOperationsEnum, dataType: "string" | "number" | "date"): ITableRows[] => {
    let filteredRows: ITableRows[] = tableRows;
    if (dataType === 'string') {
    ...
    if (dataType === "number") {
        ...
        if (operation === FilterOperationsEnum.LESS_THEN) {
            filteredRows = [...tableRows].filter(row => Number(row[column].key) < Number(value));
        }
        if (operation === FilterOperationsEnum.GREATER_THEN) {
            filteredRows = [...tableRows].filter(row => Number(row[column].key) > Number(value));
        }
    }
    if (dataType === 'date') {
...
export const handleClearFilter = (column: string, appliedFilter: Map<string, IAppliedFilter>, initialTableRows: ITableRows[]): { appliedFilter: any, filteredRows: ITableRows[] } => {
...
export const FilterOperations: IFilterOperations[] = [
    ...
    { value: FilterOperationsEnum.LESS_THEN, text: 'Less Then' },
    { value: FilterOperationsEnum.GREATER_THEN, text: 'Greater Then' }
];

// its phandling  table item search function
export const handleItemSearch = (searchVal: string, tableRows: ITableRows[], searchItemBy: ITableItemSearchState[]) => {
